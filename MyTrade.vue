  ...
  <div class="my-trade">
    ...
    <div class="data">
      <div class="header">
        <van-cell>
          <van-row>
            <van-col :span="item.key === 'time'? 10: 7" v-for="(item, index) in headerData" :key="index">
              <template v-if="item.filter"
                ><van-dropdown-menu :class="{'time-menus': item.key === 'time'}">
                  <timeFilter
                    v-if="item.key === 'time'"
                    :filters="item.filter"
      ...
      <div class="content">
        <van-list>
          <template v-if="list.length">
            <van-cell center v-for="(item, i) in list" :key="i" @click="goTradeDetail(item)">
              <template #title>
